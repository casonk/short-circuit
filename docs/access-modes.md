...
SSH access through the tunnel works when the `wireguard` firewalld zone
explicitly allows the SSH service.
...
The installer assigns the WireGuard interface to the `wireguard` firewalld zone
by default. Add only the services peers should reach, such as:

- SMB, DNS, HTTPS, and SSH.
- Explicit direct ports such as RDP when required.

To use a different zone, pass `--firewall-zone <zone>` and manually add the
required services. Avoid `trusted`, whose ACCEPT target bypasses service
allowlists.
